...
/// </summary>
public class KafkaExecutionStrategyFactory : IExecutionStrategyFactory
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public KafkaExecutionStrategyFactory(ExecutionStrategyDependencies dependencies)
    {
        Dependencies = dependencies;

        _createExecutionStrategy = dependencies.Options.FindExtension<KafkaOptionsExtension>()?.ExecutionStrategyFactory
    ...
    protected virtual IExecutionStrategy CreateDefaultStrategy(ExecutionStrategyDependencies dependencies)
        => new KafkaExecutionStrategy(dependencies);
